...
# data import
LENGTHS = os.path.join('..', 'lengths.pkl')
            ...
            for key in the_dict[words]:
                if val_string(key) and valid(words, key):
                    if key not in ret_dict:
                        ret_dict = list()
                    ret_dict.append([key, the_dict[words][key]])    # NOTE: Decide on the weight of the choice
...
if __name__ == '__main__':
    ...
    with open(LENGTHS, "rb") as hold:
        lengths = pickle.load(hold)
